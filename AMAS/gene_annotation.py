...
CHARCOUNT_COMB_DF_ALL = compress_pickle.load(os.path.join(cn.REF_DIR, 'charcount_gene_df_scaled.lzma'),
                                         compression="lzma")
...
class GeneAnnotation(object):
  ...
  def getCScores(self,
                 ...
                 mssc,
                 cutoff,
                 reference_df=CHARCOUNT_COMB_DF_ALL):
    """
    Compute the cScores
    ...
    ref_df: DataFrame
        Reference database
    info_df: DataFrame
        Reference information sharing the index with ref_df 
    ...
    ref_df = tools.filter_taxonomy(reference_df, tax).iloc[:, :-2]
    info_df = tools.filter_taxonomy(reference_df, tax).iloc[:, -2:]
  ...
  def updateGeneWithRecommendation(self, inp_recom):
    ...
    inp_recom: cn.Recommendation
       A namedtuple. Created by recom.getGeneAnnotation
...
################### ES score (not used)###################
  ...
  def getEScores(self,
    ...
    """
    # Filter the reference dataframe based on the taxonomy
    ref_df = tools.filter_taxonomy(reference_df, tax)
    synonyms_dict = prepare_synonyms_dict(ref_df)
